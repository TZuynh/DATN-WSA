        ...
        <div class="card-body">
            ...
            <form method="POST" action="{{ route('giangvien.bien-ban-nhan-xet.store', $deTai->id) }}">
                ...
                <div class="mb-3">
                    ...
                    <textarea name="hinh_thuc" class="form-control" rows="2">{{ old('hinh_thuc') }}</textarea>
                </div>
                <div class="mb-3">
                    <label class="fw-bold">2.2. Tính cấp thiết của đề tài:</label>
                    <textarea name="cap_thiet" class="form-control" rows="2">{{ old('cap_thiet') }}</textarea>
                </div>
                <div class="mb-3">
                    <label class="fw-bold">2.3. Mục tiêu và nội dung:</label>
                    <textarea name="muc_tieu" class="form-control" rows="2">{{ old('muc_tieu') }}</textarea>
                </div>
                <div class="mb-3">
                    <label class="fw-bold">2.4. Tổng quan tài liệu và tài liệu tham khảo:</label>
                    <textarea name="tai_lieu" class="form-control" rows="2">{{ old('tai_lieu') }}</textarea>
                </div>
                <div class="mb-3">
                    <label class="fw-bold">2.5. Phương pháp nghiên cứu:</label>
                    <textarea name="phuong_phap" class="form-control" rows="2">{{ old('phuong_phap') }}</textarea>
                </div>
                <div class="mb-3">
                    <label class="fw-bold">2.6. Kết quả đạt được:</label>
                    <textarea name="ket_qua" class="form-control" rows="2">{{ old('ket_qua') }}</textarea>
                </div>
                <div class="mb-3">
                    <label class="fw-bold">Quá trình hoạt động đề tài</label>
                    <textarea name="qua_trinh_hoat_dong" class="form-control" rows="2">{{ old('qua_trinh_hoat_dong') }}</textarea>
                </div>
                <div class="mb-3">
                    ...
                    <table class="table table-bordered align-middle" id="cau-hoi-table">
                        ...
                        <tbody>
                            ...
                            @else
                                <tr>
                                    <td><textarea name="cau_hoi[]" class="form-control" required></textarea></td>
                                    <td class="text-center"><button type="button" class="btn btn-danger btn-sm remove-row">X</button></td>
                                </tr>
                            @endif
                        </tbody>
