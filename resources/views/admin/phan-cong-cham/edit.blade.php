    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('admin.phan-cong-cham.update', $phanCongCham->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="form-group">
                            ...
                            <select name="de_tai_id" id="de_tai_id" class="form-control @error('de_tai_id') is-invalid @enderror" required>
                                ...
                                @foreach($deTais as $deTai)
                                    <option value="{{ $deTai->id }}" {{ old('de_tai_id', $phanCongCham->de_tai_id) == $deTai->id ? 'selected' : '' }}>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="lich_cham" id="lich_cham" class="form-control @error('lich_cham') is-invalid @enderror" placeholder="Chọn lịch chấm" value="{{ old('lich_cham', $phanCongCham->lich_cham ? $phanCongCham->lich_cham->format('Y-m-d H:i') : '') }}" required>
                            @error('lich_cham')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
